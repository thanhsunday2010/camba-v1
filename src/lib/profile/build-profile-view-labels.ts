import type { StudentProfileViewLabels } from "@/components/profile/student-profile-view";
import type { buildMockTestPageLabels } from "@/lib/mock-tests/mock-test-labels";
import type { buildSharedAchievementLabels } from "@/lib/achievements/achievement-i18n";

type ProfileT = (key: string) => string;
type JourneyT = (key: string) => string;

export function buildStudentProfileViewLabels(params: {
  t: ProfileT;
  ta: ProfileT;
  tmc: ProfileT;
  tj: JourneyT;
  mockLabels: Awaited<ReturnType<typeof buildMockTestPageLabels>>;
  achievementShared: ReturnType<typeof buildSharedAchievementLabels>;
  resolveMilestone: (titleKey: string) => string;
}): StudentProfileViewLabels {
  const { t, ta, tmc, tj, mockLabels, achievementShared, resolveMilestone } = params;
