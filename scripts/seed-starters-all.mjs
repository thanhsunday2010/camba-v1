#!/usr/bin/env node
/**
 * Seed all Starters units (1–10) from data/content/starters/*.json
 *
 * Usage:
 *   npm run seed:starters
 *   npm run seed:starters -- --from 3 --to 5
 *   npm run seed:starters -- --unlock-test-student
 */
...
function resolveContentPath(unitNumber) {
  ...
  const path = resolve(
    ROOT,
    `data/content/starters/unit-${padded}-${curriculumUnit.slug}.json`
  );
  if (!existsSync(path)) {
...
async function main() {
  ...
  console.log(`Seeding Starters units ${from}–${to}...\n`);
