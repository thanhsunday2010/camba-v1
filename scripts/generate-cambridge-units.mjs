...
const SUPPORTED_LEVELS = new Set(["starters", "movers", "flyers", "ket", "pet"]);

function parseArgs() {
  const [levelArg, fromArg, toArg] = process.argv.slice(2);
  if (!levelArg || !SUPPORTED_LEVELS.has(levelArg)) {
    throw new Error(
      `Usage: node scripts/generate-cambridge-units.mjs <starters|movers|flyers|ket|pet> [fromUnit] [toUnit]`
    );
  }
