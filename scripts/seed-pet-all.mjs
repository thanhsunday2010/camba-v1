#!/usr/bin/env node
/**
 * Seed PET units from data/content/pet/*.json
 *
 * Usage:
 *   npm run seed:pet
 *   npm run seed:pet -- --from 1 --to 3
 *   npm run seed:pet -- --unlock-test-student
 */
...
function resolveContentPath(unitNumber) {
  const curriculumUnit = getCurriculumUnit("pet", unitNumber);
  const padded = String(unitNumber).padStart(2, "0");
  const path = resolve(
    ROOT,
    `data/content/pet/unit-${padded}-${curriculumUnit.slug}.json`
  );
  if (!existsSync(path)) {
...
async function main() {
  ...
  console.log(`Seeding PET units ${from}–${to}...\n`);
